...
pnpm add github:javidjamae/AntiSlop#v0.6.0

# or run without installing
npx github:javidjamae/AntiSlop#v0.6.0 file.md --strict
```
...
`rules` accepts either the camelCase key or the rule ID as printed in findings, so `"arrow-symbol": false` and `"arrows": false` are equivalent. An unrecognized name exits 2 with the list of valid ones rather than being ignored, and so does an unrecognized top-level key. Two are exempt, because JSON has nowhere else to put them: `$schema`, which is how an editor offers completion, and any key starting with `//`, which is a note to the next reader. A `//` key works at the top level and inside `rules` and `severities`. The project publishes no config schema yet, so `$schema` is accepted and ignored rather than fetched: point it at whatever your editor expects, or leave it out. `profile` sets the base, `rules` overrides per rule, and the phrase lists take either an `add`/`remove` object (edits the defaults) or a plain array (replaces them). `--strict` on the CLI overrides the config's profile; `--config=path` pins a config explicitly. The same shapes are available in the API through `resolveConfig()`.
